...
with open('devoir2_code/filtered_countries_data.csv', 'r') as f:
  reader = csv.reader(f)
  rows = list(reader)
...
for col in data.T[1:]:
  ...
  # Replace the string 'NA' with np.nan
  col = np.where(col == 'NA', np.nan, col)
...
# Transpose the results list and convert it back to a list of lists
results_format = np.array(results).T.tolist()

# Insert the header row at the beginning of the list
